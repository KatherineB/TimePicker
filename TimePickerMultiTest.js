...
function displayPicker(id_num){
	...
	*/
	...
	alert(id_num);
	var mainViewTP = TPMV_list[id_num];
	alert(mainViewTP);
	...
	if(vertDiff < MINIMUM_ALLOWABLE_VERTICAL_DIFF){
	     document.getElementById('tpmv1').style.marginTop = VERTICAL_ADJUSTMENT;
	}
	else{
		document.getElementById(mainViewTP).style.marginTop = '0';
	}
	...
	if(horizDiff < MINIMUM_ALLOWABLE_HORIZONTAL_DIFF){
		...
		document.getElementById(mainViewTP).style.marginLeft = '0';
	}
